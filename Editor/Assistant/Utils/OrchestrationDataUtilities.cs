...
namespace Unity.AI.Assistant.Editor.Utils
{
    static class OrchestrationDataUtilities
    {
            ...
            EditorContextReport editorContextReport)
        {
            ...
            var span = Trace.StartSpan("assistant.attached_context.convert", new TraceEventOptions
            {
                Category = "assistant",
                Data = new { sourceCount = editorContextReport?.AttachedContext?.Count ?? 0 }
            });
            var contextList = new List<ChatRequestV1.AttachedContextModel>();
            var imageCount = 0;
            var annotationMaskCount = 0;
            var textCount = 0;
            var folderCount = 0;
            var cachedCount = 0;
            ...
            if (editorContextReport?.AttachedContext == null)
            {
                PayloadStats.RecordCoverage(
                    "context_conversion",
                    "OrchestrationDataUtilities.FromEditorContextReport",
                    meta: new
                    {
                        sourceCount = 0,
                        outputCount = 0
                    ...
                    options: new PayloadStatOptions
                    {
                        ...
                        ExtraFields = new
                        {
                            ...
                            outputCount = 0,
                            imageCount = 0,
                            annotationMaskCount = 0,
                            textCount = 0,
                            folderCount = 0,
                            cachedCount = 0
                        }
                    });
                span.End(new { success = true, sourceCount = 0, outputCount = 0 });
                return contextList;
            }
            ...
            try
            {
                    ...
                    switch (selection)
                    {
                        ...
                        case VirtualContextSelection virtualContext:
                        {
                            if (virtualContext.Metadata is ImageContextMetaData imageContextMetaData)
                            {
                                ...
                                var screenshotCorrelationId = Guid.NewGuid().ToString();
                ...
                PayloadStats.RecordCoverage(
                    ...
                    meta: new
                    {
                        sourceCount = editorContextReport.AttachedContext.Count,
                        outputCount = contextList.Count
                    },
                    options: new PayloadStatOptions
                    {
                        EventKind = "context_conversion",
