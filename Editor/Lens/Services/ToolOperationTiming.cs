...
namespace Becool.UnityMcpLens.Editor.Services
{
    class ToolOperationTiming
    {
        sealed class StageScope : IDisposable
        {
            readonly ToolOperationTiming m_Owner;
            readonly string m_Stage;
            readonly Stopwatch m_Stopwatch;
            bool m_Disposed;

            public StageScope(ToolOperationTiming owner, string stage)
            {
                m_Owner = owner;
                m_Stage = stage;
        ...
        public ToolOperationTiming(string toolName, string action, int requestBytes)
        {
            m_ToolName = toolName;
            m_Action = action;
