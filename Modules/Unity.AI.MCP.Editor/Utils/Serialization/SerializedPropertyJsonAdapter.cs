...
namespace Unity.AI.MCP.Editor.Utils.Serialization
{
    class SerializedPropertyJsonAdapter : IJsonAdapter<SerializedProperty>
    {
        ...
        {
            context.Writer.WriteValue($"(Type: RenderingLayerMask, Value: {prop.uintValue})");
        }

        void WriteArrayProperty(in JsonSerializationContext<SerializedProperty> context, SerializedProperty prop, int depth)
        {
            using (context.Writer.WriteArrayScope())
            {
                var arraySize = prop.arraySize;
                var limit = MaxArrayElements < 0 ? arraySize : Mathf.Min(arraySize, MaxArrayElements);

                for (var i = 0; i < limit; i++)
                {
                    var elementProp = prop.GetArrayElementAtIndex(i);
                    WriteProperty(context, elementProp, depth + 1, false);
                }

                if (MaxArrayElements > 0 && arraySize > MaxArrayElements && IndicateDepthTruncation)
                {
                    context.Writer.WriteValue($"... and {arraySize - MaxArrayElements} more");
                }
            }
        }

        void WriteObjectReference(in JsonSerializationContext<SerializedProperty> context, Object obj)
        {
            if (obj == null)
            {
                context.Writer.WriteNull();
            ...
            {
                context.Writer.WriteValue($"({obj.GetType().Name}: {obj.name})");
            }
        }
        ...
        void WriteEnumValue(in JsonSerializationContext<SerializedProperty> context, SerializedProperty prop)
        {
            ...
            var enumDisplayNames = prop.enumDisplayNames;

            if (enumIndex >= 0 && enumIndex < enumDisplayNames.Length)
            {
                context.Writer.WriteValue(enumDisplayNames[enumIndex]);
            }
            else
            {
                context.Writer.WriteValue(enumIndex);
            }
        }

        static void SafeWrite(in JsonWriter writer, float value)
        {
            if (float.IsNaN(value) || float.IsInfinity(value))
                writer.WriteValue(0f);
            else
