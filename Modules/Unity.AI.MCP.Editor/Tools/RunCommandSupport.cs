...
namespace Unity.AI.MCP.Editor.Tools.RunCommandSupport
{
    ...
    static class LensRunCommandCompiler
    {
        ...
        internal const string DynamicCommandNamespace = "Unity.AI.MCP.Editor.Tools.RunCommandSupport";
        ...
        static readonly string[] k_LegacyRunCommandUsingNamespaces = { "Unity.AI.Assistant.Editor.RunCommand" };
        ...
        static SyntaxTree ParseIntoCommandNamespace(string code)
        {
            ...
            var root = RemoveLegacyRunCommandUsings(tree.GetCompilationUnitRoot());
        ...
        static CompilationUnitSyntax RemoveLegacyRunCommandUsings(CompilationUnitSyntax root)
        {
            var legacyUsings = root.DescendantNodes()
                .OfType<UsingDirectiveSyntax>()
                .Where(usingDirective =>
                    usingDirective.Alias == null &&
                    k_LegacyRunCommandUsingNamespaces.Contains(usingDirective.Name?.ToString(), StringComparer.Ordinal))
                .ToArray();

            return legacyUsings.Length == 0
                ? root
                : (CompilationUnitSyntax)root.RemoveNodes(legacyUsings, SyntaxRemoveOptions.KeepNoTrivia);
        }
