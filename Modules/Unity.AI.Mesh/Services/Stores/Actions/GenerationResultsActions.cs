    ...
    static class GenerationResultsActions
    {
        ...
        static async Task CopySelectedFbxOrGlbToAssetsFolder(MeshResult selectedResult, AssetReference asset)
        {
            try
            {
                ...
                // Always use "selected" as the filename to avoid conflicts with preview imports
                var destModelPath = Path.Combine(assetsFolder, $"selected{extension}");
                ...
                {
                    MeshResultExtensions.ConfigureFbxImporter(modelImporter);
                    AssetDatabase.WriteImportSettingsIfDirty(destModelPath);
                    MeshResultExtensions.ExecuteWithTempDisabledErrorPause(() => AssetDatabase.ImportAsset(destModelPath, ImportAssetOptions.ForceUpdate));
                }
            }
            catch (Exception e)
            {
