...
## Lens helper recovery while the editor is still healthy
- Signals: sync/wait helpers degrade during reload churn, but direct `Unity.GetLensHealth` plus `Unity_ManageEditor GetCompactState` probes still succeed and show a healthy idle editor/runtime.
- Meaning: the helper session or wait path went stale during reload churn, but Unity itself is healthy enough to continue on the Lens path after a fresh session recovery probe.
- Action: Treat this as recovered helper churn, continue on the Lens helper path, and do not notify the user unless the direct Lens recovery probes also fail.
...
- Signals: `C:\Users\<user>\.codex\config.toml` points `unity-mcp` directly at `relay_win.exe --mcp`; Lens helper checks succeed but the live Codex MCP tool still times out.
- Meaning: Codex is bypassing the local Lens stdio transport that should own helper-script traffic.
- Action: If direct relay experimental mode is not enabled in `C:\Users\<user>\.codex\unity-mcp-settings.json`, switch back to the Lens transport. If the experiment is enabled, treat this as an intentional test configuration and benchmark stability before keeping it.
...
- Lens helper recovery still works, which can make the failure look like a Unity bridge issue even when the Codex desktop client is the broken layer.
