...
- Prefer `Unity.Tools.Menu` for compact pack-oriented navigation and `Unity.Tools.List` for compact host-visible tool names. In `UNITY_MCP_LENS_TOOL_SURFACE_MODE=static_all`, all enabled real tools are natively exposed up front and `Unity.SetToolPacks` is a compatibility no-op.
...
- Prefer `Unity.Runtime.InvokeComponentMethod`, `Unity.Runtime.SetComponentProperty`, and `Unity.Runtime.AddTemporaryComponent` for explicit Play Mode smoke actions before writing project-specific `Unity.RunCommand` snippets. Keep invocation public-instance-only by default, and use `[LensCallable]` / `[LensSmokeAction]` plus `requireLensCallable=true` for stricter project trust boundaries.
